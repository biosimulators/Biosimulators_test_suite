...
class SyntheticCombineArchiveTestCase(TestCase):
    ...
    def eval(self, specifications, synthetic_archives_dir=None, dry_run=False, cli=None):
        ...
        """
        try:
            return_value = self._eval(specifications, synthetic_archives_dir=synthetic_archives_dir, dry_run=dry_run, cli=cli)
        except Exception as exception:
    ...
    def _eval(self, specifications, synthetic_archives_dir=None, dry_run=False, cli=None):
        """ Evaluate a simulator's performance on a test case
        ...
        try:
            for i_archive, expected_results_of_synthetic_archive in enumerate(expected_results_of_synthetic_archives):
                if self._eval_synthetic_archive(specifications, expected_results_of_synthetic_archive, shared_archive_dir,
                                                i_archive, synthetic_archives_dir=synthetic_archives_dir, dry_run=dry_run,
                                                cli=cli):
                    has_warnings = True
        finally:
            shutil.rmtree(temp_dir)
    ...
    def _eval_synthetic_archive(self, specifications, expected_results_of_synthetic_archive, shared_archive_dir,
                                i_synthetic_archive, synthetic_archives_dir=None, dry_run=False, cli=None):
